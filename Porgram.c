...
{
	u8 Local_u8ErrorState = STD_TYPES_OK,Local_Pin_Value;
	if((Copy_u8PortId <= DIO_u8_PORTD) && (Copy_u8PinId <= DIO_u8_PIN7) && (Copy_Pu8ReturnedPinValue != NULL)) 
	{
		switch (Copy_u8PortId)
		{
			case DIO_u8_PORTA:
			     /*هو هنا عمل القصة ديه بدل ما يحط الفاليو في البوينتر علطول عشان لما ييجي
				                      يكتب الكود يتعامل مع كلمة high and low from macros 
									  عشان لو حاطط قيم مختلفة للهاي واللو غير الصفر والواحد */
				Local_Pin_Value=GET_BIT(DIO_u8_PINA_REG,Copy_u8PinId);
				if(Local_Pin_Value==0)
				{
			...
			case DIO_u8_PORTB:
				...
				Local_Pin_Value=GET_BIT(DIO_u8_PINB_REG,Copy_u8PinId);
				if(Local_Pin_Value==0)
				{
					*Copy_Pu8ReturnedPinValue = DIO_u8_LOW;
				}
			...
			case DIO_u8_PORTC:
				...
				Local_Pin_Value=GET_BIT(DIO_u8_PINC_REG,Copy_u8PinId);
				if(Local_Pin_Value==0)
				{
					*Copy_Pu8ReturnedPinValue = DIO_u8_LOW;
				}
			...
			case DIO_u8_PORTD:
				...
				Local_Pin_Value=GET_BIT(DIO_u8_PIND_REG,Copy_u8PinId);
				if(Local_Pin_Value==0)
				{
					*Copy_Pu8ReturnedPinValue = DIO_u8_LOW;
				}
	...
	if((Copy_u8PortId <= DIO_u8_PORTD) && ((Copy_u8PortDirection == DIO_u8_OUTPUT)||(Copy_u8PortDirection == DIO_u8_INPUT)))
	{
		switch (Copy_u8PortId)
		{
			case DIO_u8_PORTA:
				...
				{
					case DIO_u8_OUTPUT:DIO_u8_DDRA_REG = 0xff;
					break;
					case DIO_u8_INPUT:DIO_u8_DDRA_REG = 0x00;
				}
			break;
			case DIO_u8_PORTB:
				switch (Copy_u8PortDirection)
				{
					case DIO_u8_OUTPUT:DIO_u8_DDRB_REG = 0xff;
					break;
					case DIO_u8_INPUT:DIO_u8_DDRB_REG = 0x00;
				}
			break;
			case DIO_u8_PORTC:
				switch (Copy_u8PortDirection)
				{
					case DIO_u8_OUTPUT:DIO_u8_DDRC_REG = 0xff;
					break;
					case DIO_u8_INPUT:DIO_u8_DDRC_REG = 0x00;
				}
			break;
			case DIO_u8_PORTD:
				switch (Copy_u8PortDirection)
				{
					case DIO_u8_OUTPUT:DIO_u8_DDRD_REG = 0xff;
					break;
					case DIO_u8_INPUT:DIO_u8_DDRD_REG = 0x00;
				}
